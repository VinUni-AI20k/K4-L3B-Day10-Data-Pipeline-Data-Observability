...
def corrupt_clean_dataframe(df: pd.DataFrame, output_log_path: str | Path) -> pd.DataFrame:
    ...
    if "published" in corrupted_df.columns and pd.api.types.is_datetime64_any_dtype(corrupted_df["published"]):
        corrupted_df["published"] = corrupted_df["published"].dt.strftime("%Y-%m-%d").fillna("1970-01-01")
